#!/usr/bin/env python3
# UCIF2-TOWER-01 — cfts线SI0塔（塔范式第六器·互激最小环一足）
...
def patrol():
    ...
    if st == 200:
        ...
        for n in names:
            ...
            elif re.search(r'OTP@all|OTP@ucif2|【S-I|军令|奉\\s*root', n, re.I):
    ...
    if st == 200 and isinstance(items, list):
        for i in items[-8:]:
            if i['name'] != '.gitkeep': events.append({'kind': 'line-inbox', 'ref': 'ucif2-formalization-kernel:' + i['name']})
    return events
...
def kimi_work(events):
    ...
    req = urllib.request.Request('https://api.moonshot.cn/v1/chat/completions',
        method='POST', data=json.dumps({
            ...
            'messages': [
                {'role': 'system', 'content': '你是 ucif2 线（ucif2 主司）/FW2C/讨论推广一跟到底）无人驿开工分身。读候件，用中文答四件:①何事②与cfts主线何干③应动何件④生债一条。简。'},
...
def main():
    ...
    receipt = {'v': 'UCIF2-TOWER-01', 'ts': ts, 'idle_in': state.get('idle', 0),
    ...
    if events and idle < MAX_IDLE:
        ...
        st, _ = api('POST', 'dispatches',
                    {'event_type': 'ucif2-tower-cascade',
...
# BOARD-VOICE-01 (ucif2)
BOARD_INTENT_RE_UCIF2 = __import__('re').compile(r'(板面|post|广播|回应|收讫|对位|认领|开工|成果|异议|报告|判|verdict)', __import__('re').I)

def board_voice_ucif2(verdict_memo, parent_ts):
    title = f'ucif2-voice-{parent_ts}.md'
    body = f'# ucif2 塔声 — {parent_ts}\n\n{verdict_memo[:2000]}\n\n#noauto'
    p = '/tmp/_bv_' + title
